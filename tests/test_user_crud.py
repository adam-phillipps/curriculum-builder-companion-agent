...
@pytest.mark.asyncio
async def test_create_user_creates_learner_profile(test_session: AsyncSession):
    ...
    user = await create_user(test_session, user_data)
    profile = await get_learner_profile(test_session, user.id)
...
async def test_get_user(test_session: AsyncSession):
    ...
    created_user = await create_user(test_session, user_data)
    retrieved_user = await get_user(test_session, created_user.id)
...
@pytest.mark.asyncio
async def test_get_user_not_found(test_session: AsyncSession):
    """Test getting a non-existent user."""
    user = await get_user(test_session, 99999)
    assert user is None
...
async def test_update_user(test_session: AsyncSession):
    ...
    updated_user = await update_user(test_session, user.id, update_data)
...
async def test_update_user_not_found(test_session: AsyncSession):
    ...
    updated_user = await update_user(test_session, 99999, update_data)
    assert updated_user is None

@pytest.mark.asyncio
async def test_create_learner_profile(test_session: AsyncSession):
    ...
    profile = await create_learner_profile(test_session, user.id, profile_data)
...
@pytest.mark.asyncio
async def test_get_learner_profile(test_session: AsyncSession):
...
@pytest.mark.asyncio
async def test_get_learner_profile_not_found(test_session: AsyncSession):
    """Test getting a non-existent learner profile."""
    profile = await get_learner_profile(test_session, 99999)
